document.addEventListener('DOMContentLoaded', () => {
    ...
    const liveReportPreview = document.getElementById('liveReportPreview');
    const criminalCountSelect = document.getElementById('criminalCount');
    const criminalSelectorBar = document.getElementById('criminalSelectorBar');
    const criminalCcLabel = document.getElementById('criminalCcLabel');
    const reportCcInput = document.getElementById('reportCc');
    const incidentLeaderInput = document.getElementById('incidentLeader');
    const identificationOfficerInput = document.getElementById('identificationOfficer');
    const searchOfficerInput = document.getElementById('searchOfficer');
    ...
    let criminalCcValues = [''];
    let activeCriminalIndex = 0;

    function getCriminalCcValues() {
        const count = Number(criminalCountSelect.value) || 1;
        const values = [...criminalCcValues];
        while (values.length < count) values.push('');
        return values.slice(0, count);
    }

    function syncCriminalCcInput() {
        const count = Number(criminalCountSelect.value) || 1;

        if (activeCriminalIndex >= count) {
            activeCriminalIndex = count - 1;
        }

        if (criminalCcLabel) {
            criminalCcLabel.textContent = `CC do criminoso ${activeCriminalIndex + 1}`;
        }

        if (reportCcInput) {
            reportCcInput.value = criminalCcValues[activeCriminalIndex] || '';
            reportCcInput.placeholder = `CC do criminoso ${activeCriminalIndex + 1}`;
        }
    }

    function renderCriminalSelectorBar() {
        const count = Number(criminalCountSelect.value) || 1;
        criminalSelectorBar.innerHTML = '';

        while (criminalCcValues.length < count) {
            criminalCcValues.push('');
        }

        if (criminalCcValues.length > count) {
            criminalCcValues = criminalCcValues.slice(0, count);
        }

        Array.from({ length: count }, (_, index) => {
            const button = document.createElement('button');
            button.className = `criminal-selector-btn ${activeCriminalIndex === index ? 'active' : ''}`;
            button.type = 'button';
            button.textContent = index + 1;
            button.addEventListener('click', () => {
                activeCriminalIndex = index;
                renderCriminalSelectorBar();
                updateLiveReport();
            });
            criminalSelectorBar.appendChild(button);
        });

        syncCriminalCcInput();
    }

    function getReportCrimeLine(c) {
        ...
        if (c.hasSubtypes && c.subtype) {
            ...
            crimeFine = base === 0 ? unitVal * c.qty : base + (unitVal * c.qty);
            line = base === 0
                ? `${formatCrimeName(c.name)} - ${c.subtype} - ${c.qty} = ${unitVal.toLocaleString()} X ${c.qty} = ${crimeFine.toLocaleString()}€`
                : `${formatCrimeName(c.name)} - ${c.subtype} - ${c.qty} = ${base.toLocaleString()} + ${unitVal.toLocaleString()} X ${c.qty} = ${crimeFine.toLocaleString()}€`;
            ...
            const extra = Math.max(0, c.qty - (c.threshold || 0));
            crimeFine = c.fine + (extra * (c.addValue || 0));
            if (c.maxFine && crimeFine > c.maxFine) crimeFine = c.maxFine;
            line = extra > 0
                ? `${formatCrimeName(c.name)} - ${c.qty} = ${(c.addValue || 0).toLocaleString()} X ${extra} = ${crimeFine.toLocaleString()}€`
                : `${formatCrimeName(c.name)} - ${c.qty} = ${crimeFine.toLocaleString()}€`;
        } else {
            const calcParts = Object.entries(c.mults || {})
                .filter(([, m]) => m.qty > 0)
                .map(([name, m]) => `${name} - ${m.val.toLocaleString()} X ${m.qty}`);
            crimeFine += Object.values(c.mults || {}).reduce((total, m) => total + (m.qty * m.val), 0);
            if (c.isOfficial) crimeFine += 15000;
            line = calcParts.length > 0
                ? `${formatCrimeName(c.name)} - ${calcParts.join(', ')} = ${crimeFine.toLocaleString()}€`
                : `${formatCrimeName(c.name)} = ${crimeFine.toLocaleString()}€`;
        }

        return { line, crimeFine };
    }

    function buildLiveReport() {
        const summary = extraNotesArea.value.trim();
        const criminalCcs = getCriminalCcValues();
        const leader = incidentLeaderInput.value.trim();
        const identification = identificationOfficerInput.value.trim();
        const search = searchOfficerInput.value.trim();

        const criminalSections = criminalCcs.map((cc, index) => {
            const subjectNumber = index + 1;
            const subjectLines = selectedCrimes
                .filter(c => (c.assignedCriminal || 1) === subjectNumber)
                .map(getReportCrimeLine);
            const totalExtra = subjectLines.reduce((total, item) => total + item.crimeFine, 0);
            const photoStart = index * 5 + 1;
            const photoLabels = ['Cad', 'Foto sujeito', 'Identificação', 'Identificação da arma', 'Bolsos do sujeito'];
            const photoLines = Array.from({ length: 5 }, (_, photoIndex) => `➙ Foto ${photoStart + photoIndex} - ${photoLabels[photoIndex]}`).join('\n');

            return `--------------------------📸 Evidências 📸----------------------------\nCC: ${cc || ''}\n${photoLines}\n-------------------------💰 Coimas Extras 💰-------------------------\nCC: ${cc || ''}\n${subjectLines.length ? subjectLines.map(item => item.line).join('\n') : '➙ Nenhuma coima extra selecionada.'}\nTotal de coimas extra - ${totalExtra.toLocaleString()}€\nTotal de coima recomendada - 0€\nTotal coimas = 0€`;
        }).join('\n\n');

        return `ESTADO DE LOS SANTOS
RELATÓRIO OFICIAL DE OCORRÊNCIA
...
Realizador da Identificação:
➙ ${identification}

Realizador da/s revista/s ao/s civil/s:
➙ ${search}`;
    }

    function updateLiveReport() {
        liveReportPreview.textContent = buildLiveReport();
    }
    ...
    function openConfigModal(crime) {
        ...
        subjectLabel.textContent = 'Atribuir a sujeito:';
        const subjectSelect = document.createElement('select');
        subjectSelect.id = 'modalSubject';
        subjectSelect.className = 'modal-select';
        const subjectCount = Math.max(1, Number(criminalCountSelect.value) || 1);
        Array.from({ length: subjectCount }, (_, index) => {
            const option = document.createElement('option');
            option.value = `${index + 1}`;
            option.textContent = `Sujeito ${index + 1}`;
            subjectSelect.appendChild(option);
        });
        subjectSelect.value = `${Math.min(Math.max(activeCriminalIndex + 1, 1), subjectCount)}`;
        modalBody.appendChild(subjectLabel);
        modalBody.appendChild(subjectSelect);
    ...
    saveModalBtn.onclick = () => {
        ...
        selectedCrimes.push({
            ...
            assignedCriminal: subjectSelect ? Number(subjectSelect.value) : 1,
            ...
            item.innerHTML = `
                <div class="reg-main">
                    <div class="reg-info">
                        ...
                        <p>${crimeFine.toLocaleString()}€ • ${crimeJail} meses • Sujeito ${c.assignedCriminal || 1}</p>
                    </div>
    ...
    clearAllBtn.onclick = () => {
        const hasReportData = [extraNotesArea, reportCcInput, incidentLeaderInput, identificationOfficerInput, searchOfficerInput]
            .some(input => input.value.trim());
        if (selectedCrimes.length === 0 && !hasReportData) return;
        if (confirm('Deseja limpar todos os registos?')) {
            selectedCrimes = [];
            extraNotesArea.value = '';
            reportCcInput.value = '';
            incidentLeaderInput.value = '';
            identificationOfficerInput.value = '';
            searchOfficerInput.value = '';
            criminalCountSelect.value = '1';
            criminalCcValues = [''];
            activeCriminalIndex = 0;
            renderCriminalSelectorBar();
            updateUI();
            renderCrimes();
        }
    ...
    generateTicketBtn.onclick = () => {
        ...
        const previewModal = document.getElementById('ticketModal');
        const previewPrintArea = document.getElementById('ticketPrintArea');
        previewPrintArea.innerHTML = '<pre class="ticket-live-report"></pre>';
        previewPrintArea.querySelector('.ticket-live-report').textContent = liveReport;
    ...
    criminalCountSelect.addEventListener('change', () => {
        renderCriminalSelectorBar();
        updateLiveReport();
    });

    reportCcInput.addEventListener('input', () => {
        criminalCcValues[activeCriminalIndex] = reportCcInput.value.trim();
        updateLiveReport();
    });

    renderCriminalSelectorBar();
    [extraNotesArea, incidentLeaderInput, identificationOfficerInput, searchOfficerInput]
        .forEach(input => input.addEventListener('input', updateLiveReport));
    updateLiveReport();
